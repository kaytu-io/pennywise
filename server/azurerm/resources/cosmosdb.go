...
// Cosmosdb is the entity that holds the logic to calculate price
// of the azurerm_cosmosdb resource types
...
func (inst *Cosmosdb) storageCosmosCostComponents(skuName string) []query.Component {
	...
	for _, g := range inst.cosmosdbAccount.Values.GeoLocation {
		if l := locationNameMapping(g.Location); l != "" {
			components = append(components, inst.storageCosmosCostComponent(
...
func (inst *Cosmosdb) backupStorageCosmosCostComponents() []query.Component {
	...
	for _, g := range inst.cosmosdbAccount.Values.GeoLocation {
		...
		if l := locationNameMapping(g.Location); l != "" {
			components = append(components, inst.backupCosmosCostComponent(
