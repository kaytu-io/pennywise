...
func (ts *AzureTestSuite) TestAutomationAccount() {
	ts.SetupSuite()
	fmt.Println("Suite Setup")
	ts.IngestService("Automation", "eastus")
	fmt.Println("Automation ingested")

	usg, err := ts.getUsage("../../testdata/azure/automation_account/usage.yaml")
	require.NoError(ts.T(), err)

	stat := ts.getDirCosts("../../testdata/azure/automation_account", *usg)
	costComponent := stat.GetCostComponents()

	expectedCostComponent := []cost.Component{
		{
			Name:            "Job run time",
			MonthlyQuantity: decimal.NewFromFloat32(10),
		...
		{
			Name:            "Job run time",
			MonthlyQuantity: decimal.NewFromFloat32(15),
			HourlyQuantity:  decimal.Zero,
			Unit:            "minutes",
			Rate: cost.Cost{
				Decimal:  decimal.NewFromFloat(0.002),
