...
func (p *dbPrice) toDomainEntity() *price2.Price {
...
func newPrice(pwp *price2.WithProduct) (*dbPrice, error) {
...
func (r *PriceRepository) Filter(ctx context.Context, products []*product.Product, filter *price2.Filter) ([]*price2.Price, error) {
	where := parsePriceFilter(filter, products)
	q := fmt.Sprintf(`
...
func (r *PriceRepository) Upsert(ctx context.Context, pwp *price2.WithProduct) (price2.ID, error) {
	p, err := newPrice(pwp)
	if err != nil {
		return 0, err
	}
	...
	return price2.ID(id), nil
}

// DeleteByProductWithKeep deletes all the prices of the product with given product.ID except the ones in the keep slice.
func (r *PriceRepository) DeleteByProductWithKeep(ctx context.Context, productID product.ID, keep []price2.ID) error {
...
func scanPrice(row sqlr.Scanner) (*price2.Price, error) {
	var p dbPrice
